...
WHERE ((CODIGO_STATUS  IN (0,11) AND FILA.ORIGEM = 'CDR') OR (CODIGO_STATUS = 3 AND FILA.ORIGEM = 'Regulação') OR (CODIGO_STATUS = 5 AND FILA.ORIGEM = 'Solicitações')) """)
    ...
    comando_2 = text("""WITH PACIENTES AS (SELECT DISTINCT CODIGO_PACIENTE FROM TB_TEMPO_CUIDAR_ATUALIZADA FILA)
                    SELECT * FROM PACIENTES
                    WHERE NOT EXISTS (SELECT 1 FROM TB_PACIENTES DADOS_PACIENTES WHERE DADOS_PACIENTES.CODIGO = PACIENTES.CODIGO_PACIENTE)""")
    



    dfCodigos = ExtrairDados(conexaoMRA, comando_2)
